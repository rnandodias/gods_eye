...
def create_event(type, name, initial_date, vacation=()):
    """
    ...
    `create_event(type, name, initial_date, vacation=())` ► Gera o JSON que será enviado para a API do Google Calendar com os planejamentos definidos para cada instrutor
    ...
    for i, step in enumerate(steps):
        if i == 0:
            ...
            final_date = initial_date + timedelta(days=deadlines[1])
...
def create_agenda(start_work_cycle, plan, instructors, last_task_or_vacation_date, ferias_compilado):
        ...
        for i in plan.query('Prioridade in ["Alta", "Média"] and `Grupo Instrutor` in @instructors and `Status Atual` not in ["Pós-produção", "Finalizado"]').sort_values(by=['Grupo Instrutor', 'Prioridade']).index:
            if plan['Grupo Instrutor'][i] == instructor:
                ...
                if plan['Tipo'][i] == 'Quinta com Dados' or plan['Tipo'][i] == 'Palestra' or plan['Tipo'][i] == 'Podcast':
                    ...
                    year, month, day = [int(item) for item in plan['Conteúdos'][i].split()[-1].replace('[', '').replace(']', '').split('-')]
                    evento, _ = create_event(plan['Tipo'][i], plan['Conteúdos'][i], str(date(year, month, day)), ferias)
                else:
                    evento, inicio = create_event(plan['Tipo'][i], plan['Conteúdos'][i], inicio, ferias)
    ...
    # with open(path + 'agendas_1_tri_2024.json', 'w') as arquivo_json:
    #     json.dump(scheduling, arquivo_json)

    return scheduling

# --------------------------------------------------------------------------------------
# Cria e configura o DataFrame que servirá de inputa para construção da timeline
